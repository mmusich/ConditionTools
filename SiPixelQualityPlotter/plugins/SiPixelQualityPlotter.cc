...
//#include "ConditionsTools/SiPixelQualityPlotter/interface/CMS_Lumi.h"
...
SiPixelQualityPlotter::SiPixelQualityPlotter(const edm::ParameterSet& iConfig)
  ...
  for (unsigned int lay = 1; lay <= 4; lay++) {
    ...
    std::string title = "; Module # ; Ladder # ; fraction of bad luminosity per component (%)";
  ...
  for (unsigned int ring = 1; ring <= n_rings; ring++) {
    ...
    std::string title = "; Disk # ; Blade/Panel # ; fraction of bad luminosity per component (%)";
...
void SiPixelQualityPlotter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (hasQualityIOV || RunNumber_ > lastRun_) {
    ...
    std::cout << "Accumulated Luminosity: " << lumiSinceLastReset_ << " | Total Luminosity: " << totalLumi_
              << std::endl;
  ...
  totalLumi_ += lumi.integLuminosity();
  lumiSinceLastReset_ += lumi.integLuminosity();
}
...
void SiPixelQualityPlotter::endJob() {
  ...
  //canvasB.SaveAs(("SummaryBarrel_"+analyzedTag_+".root").c_str());
  ...
  //canvasF.SaveAs(("SummaryForward_"+analyzedTag_+".root").c_str());
}
